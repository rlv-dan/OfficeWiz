...
Public bPortableMode As Boolean

Public Sub InitPortable()
    If Dir(App.Path & "\PORTABLE", vbArchive + vbHidden + vbReadOnly + vbSystem) <> "" Then bPortableMode = True Else bPortableMode = False
End Sub
...
Public Function GetSettingEX(sPath As String, sApp As String, sVar As String, varDeafult As Variant) As Variant

    If bPortableMode = True Then
        GetSettingEX = ReadIniValue("settings.ini", sApp, sVar, CStr(varDeafult))
    Else
        GetSettingEX = GetSetting(sPath, sApp, sVar, varDeafult)
    End If

End Function

Public Sub SaveSettingEX(sPath As String, sApp As String, sVar As String, sValue As Variant)

    If bPortableMode = True Then
        WriteIniValue "settings.ini", sApp, sVar, sValue
    Else
        SaveSetting sPath, sApp, sVar, sValue
    End If
